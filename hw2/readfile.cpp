      ...
      if ((str.find_first_not_of(" \t\r\n") != string::npos) && (str[0] != '#')) {
        ...
        if (cmd == "light") {
          if (numused == numLights) { // No more Lights 
            ...
            if (validinput) {
              ...
              for (i = 0; i < 4; ++i){
                lightposn[4 * numused + i] = values[i]; // direction
                lightcolor[4 * numused + i] = values[i+4]; // color
              }
        ...
        // Note that no transforms/stacks are applied to the colors. 
          ...
          if (validinput) {
            ...
            for (int i = 0; i < 3; i++) eyeinit[i] = values[i];
            for (int i = 0; i < 3; i++) center[i] = values[i+3];
            for (int i = 0; i < 3; i++) upinit[i] = values[i+6];
            fovy = values[9];
            // modelview = Transform::lookAt(eyeinit, center, upinit);
            // float aspect = w / h, zNear = 0.1, zFar = 99.0; 
